...
  // IMPORTANT: Check if toast element exists before trying to use it
  if (!toast) return; 
...
const propertyImageInput = document.getElementById('propertyImage'); // Added input reference
...
    // If a type is selected, filter by it
    if (type && (p.type || '') !== type) return false;
    
    // **FIXED: Status Filtering Logic**
    const filterByStatus = free || taken; 
    if (filterByStatus) {
        const statusMatches = (free && p.status === 'free') || (taken && p.status === 'taken');
...
    // IMPORTANT: Check if status exists before trying to display it
...
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json', 'role': role, 'Authorization': `Bearer ${username}` }, // Added Authorization
    });
    // **FIXED: Backend might not return JSON on DELETE**
...
  
  // **FIXED: Use correct image input variable**
  let image = '';

  if (propertyImageInput.files.length > 0) {
    // **FIXED: Use utility for clean async file reading**
    image = await readFileAsDataURL(propertyImageInput.files[0]);
...
      method,
      // Added Authorization header for backend checks
      headers: { 'Content-Type': 'application/json', 'role': role, 'Authorization': `Bearer ${username}` }, 
      body: JSON.stringify(property), // **FIXED: Send 'property' object directly, not wrapped in { property: ... }**
    });
    const data = await res.json();
    if (data.success) {
...
/* ---------- Add Property Modal Handlers (FIXED) ---------- */
openAddBtn.addEventListener('click', () => {
    editingPropertyId = null; // Ensure new property is being added
    propertyForm.reset();
    propertyImageInput.required = true; // Image is likely required for new property
    addPropertyModal.querySelector('h2').textContent = 'Добави нов имот';
    openModal(addPropertyModal);
});

// Reset logic and close handler for the Add/Edit Modal
function resetAddEditModal() {
    editingPropertyId = null;
    propertyForm.reset();
    propertyImageInput.required = false;
    addPropertyModal.querySelector('h2').textContent = 'Добави нов имот';
    closeModal(addPropertyModal);
}

closeAdd.addEventListener('click', resetAddEditModal);


/* ---------- Global Modal Close Listener (FIXED/IMPROVED) ---------- */
// This listener fixes the "pop up for adding/editing property doesnt work+ X to close it doesnt work"
window.addEventListener('click', e => {
  // Check against the modal background (assuming CSS makes the modal cover the whole window)
  if (e.target === loginModal) closeModal(loginModal);
  // When closing the Add/Edit modal, we must reset the state
  if (e.target === addPropertyModal) resetAddEditModal(); 
...
// The line below was syntactically incorrect and unnecessary without the definition of 'style'. Deleted.
